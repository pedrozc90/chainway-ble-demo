...
public class CheckUtils {
    ...
    public static int getInsertIndex(List<UHFTAGInfo> listData, UHFTAGInfo newInfo, boolean[] exists) {
        ...
        while (true) {
            ...
            ret = compareBytes(
                    (newInfo.getReserved() + newInfo.getEPC() + newInfo.getTid()).getBytes(),
                    (listData.get(judgeIndex).getReserved() + listData.get(judgeIndex).getEPC() + listData.get(judgeIndex).getTid()).getBytes()
            );
            if (ret > 0) {
